...
from janus.dataset import PerNFDataset
...
def _subset_dataset(ds: PerNFDataset, indices: list[int]) -> PerNFDataset:
...
def _split_dataset(ds: PerNFDataset, val_pct: float) -> tuple[PerNFDataset, PerNFDataset]:
...
def _dataset_label_stats(ds: PerNFDataset) -> tuple[int, int]:
...
def _log_sample_pairs(ds: PerNFDataset, name: str) -> None:
    """Log one example normal/anomaly pair per NF for ``ds``."""
    if not getattr(ds, "pairs_by_nf", None):
        return
    for nf, pairs in ds.pairs_by_nf.items():
        if not pairs:
            continue
...
def _log_pair_counts(ds: PerNFDataset, name: str) -> None:
...
def main() -> None:
    ...
    data_root = resolve_path(repo_cfg.get("data_root", "data/raw_data"))
    logs_dir = resolve_path(
        repo_cfg.get("clean_logs_dir", "data/raw_data/logs"))
    anom_dir = resolve_path(
        repo_cfg.get("anomalous_logs_dir", "data/raw_data/anomalous_logs"))
    root = logs_dir if exclude_anomalous_logs else data_root
    ...
    anomaly_meta = None
    if not exclude_anomalous_logs:
        anomaly_meta = anom_dir / "metadata.json"
        if not anomaly_meta.exists():
            anomaly_meta = None
        else:
            logger.info("[DATA] Loaded anomaly metadata from %s", anomaly_meta)

    if exclude_anomalous_logs:
        logger.info("[DATA] Clean logs from %s; anomalous logs excluded",
                    logs_dir)
    elif anomaly_meta is None:
        logger.info("[DATA] Clean logs from %s", logs_dir)
        logger.info("[DATA] Anomalous logs will be generated synthetically")
    else:
        if keep_clean_in_anomalous:
            logger.info(
                "[DATA] Clean logs from %s and clean sections in %s",
                logs_dir,
                anom_dir,
            )
        else:
            logger.info("[DATA] Clean logs from %s", logs_dir)
        logger.info("[DATA] Anomalous logs from %s", anom_dir)
